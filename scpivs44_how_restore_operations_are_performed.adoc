...
IMPORTANT: 在還原作業期間、您可能會遇到此錯誤、或 SnapCenter VMware 外掛程式嘗試重新簽署複本時發生此錯誤 `Host unresolved volumes is null` `Exception while calling pre-restore on SCV….Error mounting cloned LUN as datastore…` 。由於VMware的限制、SnapCenter 在進階ESXi組態中、VMware插件無法控制自動重新簽名值。
