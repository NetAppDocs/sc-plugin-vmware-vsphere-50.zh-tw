...
+
若要取得權杖、請使用 `/4.1/auth/login` REST API 並提供 vCenter 認證。

* API版本指定
+
每個REST API名稱都包含SnapCenter 最先發行REST API的版本編號。例如、 REST API `/4.1/datastores/{moref}/backups` 最初是在 SnapCenter 4.1 中發行。
